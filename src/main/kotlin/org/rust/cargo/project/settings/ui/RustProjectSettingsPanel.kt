...
class RustProjectSettingsPanel(
    ...
    fun validateSettings() {
        val toolchain = sdk?.toolchain ?: return
        if (!toolchain.looksLikeValidToolchain()) {
            throw ConfigurationException("Invalid toolchain location: ${toolchain.location}")
        }
    }
    ...
    private fun updateSdkListAndSelect(selectedSdk: Sdk?) {
        ...
        for ((idx, currentSdkType) in RsRenderedSdkType.values().withIndex()) {
            if (currentSdkType in moduleSdksByTypes) {
                if (idx != 0) items.add(RsSdkListCellRenderer.SEPARATOR)
